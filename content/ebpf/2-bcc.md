...
1. 内核态使用 C 语言开发，bcc 提供了一系列的 [库函数与宏定义](https://github.com/iovisor/bcc/blob/master/docs/reference_guide.md) 来简化开发。
2. 用户态主要负责包含 eBPF 程序加载、挂载到内核函数和跟踪点，以及通过 BPF 映射获取和打印执行结果等部分，不限制开发语言，bcc 提供了一系列的 python 函数来简化这些操作。
