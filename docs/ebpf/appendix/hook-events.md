...
| hook 类型  | eBPF 程序类型  | 回调时机                       |
| :--------- | :------------- | :----------------------------- |
| 内核函数   | kprobe         | 内核空间函数调用开始           |
| 内核函数   | kretprobe      | 内核空间函数调用结束           |
| 内核跟踪点 | tracepoint     | 内核跟踪点被执行               |
| 内核跟踪点 | raw_tracepoint | 内核跟踪点被执行（不解析参数） |
| 网络接口   | xdp            | 网络接口收发数据包             |
| 用户函数   | uprobe         | 用户空间函数调用开始           |
| 用户函数   | uretprobe      | 用户空间函数调用结束           |
| 网卡       | raw_socket     | 网卡收包                       |
